...
planets %>%
  ...
  summarise(total_discoveries = sum(number))
...
top_stars <- planets %>%
  group_by(star) %>%
  summarise(total_planets = sum(number)) %>%
  distinct(star, .keep_all = TRUE) %>%
  slice_max(order_by = total_planets, n = 5)
...
The position_dodge2(preserve = "single") setting in the bar plot ensures that bars representing different categories (in this case, discovery methods) within the same group (system size) are placed side by side with consistent width, even if the number of categories varies across groups.In contrast, using position = "dodge" also places bars side by side but may result in inconsistent bar widths when the number of categories varies across groups.position_dodge2(preserve = "single") offers a more visually consistent and easier-to-interpret representation of the data, especially when comparing categories across groups with varying numbers of categories.
...
```{r}
missing_values <- planets %>%
  filter(method %in% c("Transit", "Radial Velocity")) %>%
  group_by(method) %>%
  summarise(missing_radius = sum(is.na(radius)),
